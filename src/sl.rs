...
impl SlApi {
    ...
    pub fn find(&self, term: &str) -> Result<FindResponse, error::Error> {
        let url = format!("https://sl.se/api/TypeAhead/Find/{term}", term = term);
    ...
    pub fn travel(
        ...
        let url = match opt {
            &TravelOption::Arrival(dt) => {
                format!("{}/{}/arrive/", base_url, dt.format("%Y-%m-%d %H_%M"))
            }
            &TravelOption::Departure(dt) => {
                format!("{}/{}/depart/", base_url, dt.format("%Y-%m-%d %H_%M"))
            }
